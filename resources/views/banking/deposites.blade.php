                ...
                <header class="header bg-white b-b clearfix">
                  <div class="row m-t-sm">
                    ...
                    <a href="#" data-toggle="modal" class="btn btn-sm btn-default"><i class="fa fa-print"></i> Print List</a>
                     <span class="badge badge-info">Record(s) Found : {{ $deposites->total() }} {{ str_plural('Deposites', $deposites->total()) }} </span>
                    </div>
                      ...
                      <table class="table table-striped m-b-none text-sm" width="100%">
                        ...
                        @foreach( $deposites as $deposite)
                          <tr>
                            ...
                            <td><a href="#modal_new_deposite" class="bootstrap-modal-form-open" onclick="editdeposite('{{ $deposite->id }}')"  id="edit" name="edit" data-toggle="modal" alt="edit"><i class="fa fa-pencil"></i></a></td>
                         
                            <td>{{ $deposite->created_on }}</td>
                            <td>{{ $deposite->bank_name }}</td>
                            <td>{{ $deposite->broker_bank_account }}</td>
                            <td>{{ $deposite->account_name }}</td>
                            <td>{{ $deposite->payment_description }}</td>
                            <td>{{ $deposite->payment_sum }}</td>
                            
                            <td>
                              <a href="#" class="active" onclick="deactivateAccount('{{ $deposite->id }}')" data-toggle="class"><i class="fa fa-check text-success text-active"></i><i class="fa fa-times text-danger text"></i></a>
                            </td> 
        ...
        <div class="modal-body">
                    ...
                    <header class="header bg-light bg-gradient">
                      <ul class="nav nav-tabs nav-white">
                       ...
                       <li class=""><a href="#fixedincometab" data-toggle="tab">New Deposite</a></li>
                      </ul>
                    </header>
                    <section class="scrollable">
                      <div class="tab-pane active" id="equitytab">
                         ...
                         <div class="tab-pane" id="fixedincometab">
                           <form  method="post" data-validate="parsley"  action="/banking.create-deposite" class="panel-body wrapper-lg">
                          @include('banking/new_deposite')
